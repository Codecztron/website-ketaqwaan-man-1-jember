document.addEventListener("DOMContentLoaded", function() {
    ...
    function createProgramCardHTML(program) {
        return `
            <div class="program-card">
                ...
                <div class="program-status status-${program.status.toLowerCase()}">${program.status}</div>
            </div>
        `;
    }
    ...
    function addStatusAnimations() {
        ...
        styleSheet.textContent = `
            ...
            .status-ongoing { animation-duration: 2.5s; }
            .status-ongoing::before { background: #2196F3; box-shadow: 0 0 12px rgba(33, 150, 243, 0.5) ; }
    
            .status-completed { animation-duration: 3s; }
            .status-completed::before { background: #4CAF50; box-shadow: 0 0 12px rgba(76, 175, 80, 0.5); }
    
            .status-planned { animation-duration: 3.5s; }
            .status-planned::before { background: #FFC107; box-shadow: 0 0 12px rgba(255, 193, 7, 0.5); }
